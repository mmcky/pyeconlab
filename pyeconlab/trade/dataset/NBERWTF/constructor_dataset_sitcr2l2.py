"""
NBER (Self Contained) Dataset Functions [SITC Revision 2 Level 2]
...
#-SITC Revision 2 Level 2-#

def construct_sitcr2l2(df, data_type, dropAX=True, sitcr2=True, drop_nonsitcr2=True, adjust_hk=(False, None), intertemp_cntrycode=False, drop_incp_cntrycode=False, adjust_units=False, source_institution='un', verbose=True):
        """
        Construct a Self Contained (SC) Direct Action Dataset for Countries at the SITC Revision 2 Level 2
        ...
        1. Operations ::
            ...
            Optional:
            ---------
            [A] Drop sitc2 codes that contain 'A' and 'X' codes [Default: True]
        ...
        df['sitc2'] = df.sitc4.apply(lambda x: x[0:2])
        df = df.groupby(['year', 'exporter', 'importer', 'sitc2']).sum()['value'].reset_index()
        ...
        if data_type == 'export' or data_type == 'exports':
            ...
            df = df.loc[(df.eiso3c != '.')]
            df = df.groupby(['year', 'eiso3c', 'sitc2']).sum()['value'].reset_index()
        #-Imports (can include NES on importer side)-#
            ...
            df = df.loc[(df.iiso3c != '.')]
            df = df.groupby(['year','iiso3c', 'sitc2']).sum()['value'].reset_index()
        #-Trade-#
        else: 
            ...
            df = df.groupby(['year', 'eiso3c', 'iiso3c', 'sitc2']).sum()['value'].reset_index()
        
        #-Remove Product Code Errors in Dataset-#
        df = df.loc[(df.sitc2 != "")]                                                                   #Does this need a reset_index?
        #-dropAX-#
        if dropAX:
            if verbose: print "[INFO] Dropping SITC Codes with 'A' or 'X'"
            df['AX'] = df.sitc2.apply(lambda x: 1 if re.search("[AX]", x) else 0)
            df = df.loc[df.AX != 1]
        ...
        if sitcr2:
            ...
            codes = sitc.get_codes(level=2)
            df['sitcr2'] = df['sitc2'].apply(lambda x: 1 if x in codes else 0)
            if drop_nonsitcr2:
        ...
        if intertemp_cntrycode:
            ...
            if data_type == 'export' or data_type == 'exports':
                ...
                df = df[df['eiso3c'] != '.']
                df = df.groupby(['year', 'eiso3c', 'sitc2']).sum().reset_index()
            #-Import-#
            elif data_type == 'import' or data_type == 'imports':
                ...
                df = df[df['iiso3c'] != '.']
                df = df.groupby(['year', 'iiso3c', 'sitc2']).sum().reset_index()
            #-Trade-#
            else:
                ...
                df = df[df['iiso3c'] != '.']
                df = df[df['eiso3c'] != '.']
                df = df.groupby(['year', 'eiso3c', 'iiso3c', 'sitc2']).sum().reset_index()
